...
stan_bivar_shglm_activity_aggression <- stanmods[stanmods$model=="bivariate shglm" & stanmods$behaviour1=="activity" & stanmods$behaviour2=="aggression",]
stan_bivar_dhglm_activity_aggression <- stanmods[stanmods$model=="bivariate dhglm" & stanmods$behaviour1=="activity" & stanmods$behaviour2=="aggression",]
...
#################################
  ...
  # (Step 1): Extract the population coefficients for mean model
	...
	# (Step 2): Extract the population coefficients for dispersion model	 
		 	 ...
		 	 mean(2*B0_sd_exp)
		 	 mean(stanproc_personality_plasticity_dhglm_aggression$B1_v0) #@ROSE: DIFFERENT, probably because the stan model doesn't include a random slope
	...
	# (Step 4): **New step as we now have a sd for the random slopes.**
	     ...
	     #@ROSE: we don't want the random slope, the dataset is not good enough for it
...
###################################
	  ...
	  # (Step 1): ** Calculate average within-individual / residual variance **
		 	 ...
		 	 mean(sigma_w)
		 	 mean(stanproc_personality_plasticity_dhglm_aggression$sigma1_w) #@ROSE: different, because this model doesn't have a random slope
		...
		# (Step 2): Calculate total phenotypic variance. Note here that we add in the random slope variance now
		 	 ...
		 	 mean(var_p)
		 	 mean(stanproc_personality_plasticity_dhglm_aggression$var1_p) #@ROSE: different
...
###################################
		...
		# (Step 2): ** Sum of IDv0 (intercept), IDv2 (slope) and fixedV = total variance in residual variance on log-normal scale. See eqn 39. in MS.** 
		 	   var_residvar_exp <- func_sum_var(vars = data.frame(sigma_ID_v0^2, var_fixed_v, sigma_ID_v2)) #@ROSE: need to change this to not include random slope
