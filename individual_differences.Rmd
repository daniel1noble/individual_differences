...
author: Rose E. O’Dea, Daniel W.A. Noble, Shinichi Nakagawa
date: "`r Sys.Date()`"
output: 
...
```
# Introduction
This supplement introduces readers to the calculations and modeling approaches reviewed by:  
Rose E. O'Dea, Daniel W.A. Noble and Shinichi Nakagawa 2021. Unifying individual differences in personality, predictability, and plasticity: a practical guide. *Methods in Ecology and Evolution*, using the `brms` R package (which interfaces with `stan`).

We only fit an exemplar set of models from our worked example. More specifically, we focus on a subset of models using two traits (aggression and activity) to demonstrate how to fit single- and double- hierarchical generalized linear models (abbreviated as SHGLM and DHGLM, respectively). This includes univariate and bivariate (two trait) models. For a selection of models we also show how to: 1) calculate repeatability and the coefficient of variation for the mean and dispersion models and 2) extract and manipulate individual BLUPs (best linear unbiased predictors) to estimate between-individual correlations outlined in our manuscript. 
...
```{r funcs, include=TRUE, message=FALSE, warning=FALSE, class.source='klippy'}
  ...
  # within-individual standard deviation with a random slope for dispersion model
	  func_var_within_slope <- function(Bpv, sigma_v0, var_fixed_v, sigma_ID_v2){
	  
	  # summed variance components
	  var_within_exp <- func_sum_var(vars = data.frame(sigma_v0^2, var_fixed_v, sigma_ID_v2))
	  # conversion back from log scale
	  var_within <- func_ln_convert(mu_ln = Bpv, sigma_ln = sqrt(var_within_exp))$mu_raw
...
Here, using the model we can calculate the personality-plasticity association (i.e., the correlation between individual intercepts and *absolute* slopes).
...
In this section we are modeling a random slope and intercept for both the mean and dispersion models. We will also estimate the correlation between slopes and intercepts across the models. 
...
##################################################################
	...
	if(rerun4){
		...
		model4_fit <- readRDS(here("models", "model4_fit"))
	}
...
```{r personality_slope_cvm_rpm, include=TRUE, message=FALSE, warning=FALSE, class.source='klippy'}
#################################
# Mean and Dispersion Paramaters
#################################
 
  # (Step 1): Extract the population coefficients for mean model
		 	 B0_m <- brms::posterior_samples(model4_fit, pars = "b_Intercept")[,1]			 
		 	 B1_m <- brms::posterior_samples(model4_fit, pars = "b_sex")[,1]	
		 	 B2_m <- brms::posterior_samples(model4_fit, pars = "b_age.Z")[,1]	
	
	# (Step 2): Extract the population coefficients for dispersion model	 
		 	 B0_sd_exp <- brms::posterior_samples(model4_fit, pars = "b_sigma_Intercept")[,1]			
		 	 B1_sd_exp <- brms::posterior_samples(model4_fit, pars = "b_sigma_sex")[,1]	
		 	 B2_sd_exp <- brms::posterior_samples(model4_fit, pars = "b_sigma_age.Z")[,1]
		 	 
	# (Step 3): Extract the random ID intercept variance components for random intercept. Note that we model ln(sd) so we need to convert to variance scale by multipling by 2.
		 	  sigma_ID_m0 <- brms::posterior_samples(model4_fit, pars = "sd_id__Intercept")[,1]
		 	 sigma_ID_sd0 <- brms::posterior_samples(model4_fit, 
		 	                                         pars = "sd_id__sigma_Intercept")[,1]
		 	  sigma_ID_v0 <- 2*sigma_ID_sd0
	
	# (Step 4): **New step as we now have a sd for the random slopes.**
	     sigma_ID_m2 <- brms::posterior_samples(model4_fit, pars = "sd_id__age.Z")[,1]
	    sigma_ID_sd2 <- brms::posterior_samples(model4_fit, pars = "sd_id__sigma_age.Z")[,1]
	     sigma_ID_v2 <- 2*sigma_ID_sd2
		 	  
	# (Step 5): Calculate the fixed effect variance for the mean model (var_fixed_m) and the dispersion model (B_pv). Note here that the calculations are the same because we assume intercapt at an average age.
		 	 B_pm <- func_Bp(B0 = B0_m, B1 = B1_m)
		 	 B_pv <- 2*func_Bp(B0 = B0_sd_exp, B1 = B1_sd_exp)
...
###################################
	  ...
	  # (Step 1): ** Calculate average within-individual / residual variance **
		 	 sigma_w <- func_var_within_slope(        Bpv = B_pv, 
		 	                               sigma_v0 = sigma_ID_v0, 
		 	                            var_fixed_v = var_fixed_v,
		 	                            sigma_ID_v2 = sigma_ID_v2)
...
###################################
		...
		# (Step 2): ** Sum of IDv0 (intercept), IDv2 (slope) and fixedV = total variance in residual variance on log-normal scale. See eqn 39. in MS.**
		 	   var_residvar_exp <- func_sum_var(vars = data.frame(sigma_ID_v0^2, var_fixed_v, sigma_ID_v2))
...
In this section we briefly show readers how they can fit a bivariate model with aggression and activity, estimating intercepts within and among the traits at the individual-level. The principles for calculating repeatability and the coefficient of variation are the same as that outlined in [Personality (SHGLM)]. 
...
rerun5 = FALSE 
	...
	if(rerun5){
		...
		model5_fit <- readRDS(here("models", "model5_fit"))
	}
...
In this section we briefly show readers how they can fit a bivariate model with aggression and activity, estimating intercepts, slopes and correlations within and among the traits. The principles for calculating repeatability and the coefficient of variation are the same as that outlined in [Personality / Predictability (DHGLM)]. 
...
####################################################################################
	...
	saveRDS(model6_fit, "./models/model6_fit")
		...
		model6_fit <- readRDS(here("models", "model6_fit"))
	}
```