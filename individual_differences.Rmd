...
# Load Libraries and relevant data
		pacman::p_load(klippy, brms, dplyr, coda, here, flextable)	
	...
	# Fit a model for aggression that includes fixed effects of sex and z-transformed age plus a random intercept for individual ID
		...
		if(rerun1) {
			...
			brms_personality_shglm_aggression <- brms::brm(formula_personality_shglm_aggression, data = dat, 
									iter = 6000, warmup = 2000, chains = 3, cores = 3, 
									save_pars = save_pars())
			saveRDS(brms_personality_shglm_aggression, "./brms_models/brms_personality_shglm_aggression") 
		} else {
			brms_personality_shglm_aggression <- readRDS(here::here("brms_models","brms_personality_shglm_aggression"))
		}
...
# Results
			personality_results <- data.frame( CVm = mean(CV_m),
			                                   CVm.L = coda::HPDinterval(coda::mcmc(CV_m))[1],
			                                   CVm.U = coda::HPDinterval(coda::mcmc(CV_m))[2],
			                                   	Rpm =  mean(Rp_m),
			                                   Rpm.L = 	coda::HPDinterval(coda::mcmc(Rp_m))[1],
			                                   Rpm.U = 	coda::HPDinterval(coda::mcmc(Rp_m))[2])
...
###############################################   	
	...
	if(rerun2){
		...
		saveRDS(brms_personality_dhglm_aggression, "./brms_models/brms_personality_dhglm_aggression")
	} else {
		brms_personality_dhglm_aggression <- readRDS(here::here("brms_models", "brms_personality_dhglm_aggression"))
	}
...
pers_pred_results <- data.frame(model = c("Mean Model", "", "Dispersion Model", ""),
                                ...
                                L.CI= c(coda::HPDinterval(coda::mcmc(Rp_mu))[1], coda::HPDinterval(coda::mcmc(CV_mu))[1], coda::HPDinterval(coda::mcmc(Rp_var))[1],coda::HPDinterval(coda::mcmc(CV_var))[1]), 
                                       U.CI= c(coda::HPDinterval(coda::mcmc(Rp_mu))[2], coda::HPDinterval(coda::mcmc(CV_mu))[2], coda::HPDinterval(coda::mcmc(Rp_var))[2],coda::HPDinterval(coda::mcmc(CV_var))[2]))
...
###################################################
	...
	if(rerun3){
		...
		brms_personality_plasticity_shglm_aggression <- brm(formula_stan_personality_plasticity_shglm_aggression, data = dat, 
								iter = 6000, warmup = 2000, chains = 3, cores = 3, 
								save_pars = save_pars()) 
		saveRDS(brms_personality_plasticity_shglm_aggression, "./brms_models/brms_personality_plasticity_shglm_aggression")
	} else {
		brms_personality_plasticity_shglm_aggression <- readRDS(here::here("brms_models", "brms_personality_plasticity_shglm_aggression"))
	}
...
```{r pers_plas, include=TRUE, message=FALSE, warning=FALSE, class.source='klippy'}
      ...
      blups_IDm2 <- brms::posterior_samples(brms_personality_plasticity_shglm_aggression, pars = "r_id[[1-9].*,age.Z]")
             Bm2 <- brms::posterior_samples(brms_personality_plasticity_shglm_aggression, pars = "b_age.Z", fixed = TRUE)
      ...
      # Using absolute slope and the individual deviations for each individuals intercept calculate the correlation across the 248 fish for each posterior sample of individual blups
        cors <- cor_calc(slopes = abs_slope, intercepts = blups_IDm0)
      ...
      # Estimate the mean and 95% CI  
         mean(cors)
         coda::HPDinterval(coda::mcmc(cors))
...
##################################################################
	...
	if(rerun4){
		...
		brms_personality_plasticity_dhglm_aggression <- brm(formula_personality_plasticity_dhglm_aggression, data = dat, 
								iter = 6000, warmup = 2000, chains = 3, cores = 3, 
								save_pars = save_pars()) 
		saveRDS(brms_personality_plasticity_dhglm_aggression, "./brms_models/brms_personality_plasticity_dhglm_aggression")
	} else {
		brms_personality_plasticity_dhglm_aggression <- readRDS(here::here("brms_models", "brms_personality_plasticity_dhglm_aggression"))
	}
...
#################################
  ...
  # (Step 1): Extract the population coefficients for mean model
	...
	# (Step 4): **New step as we now have a sd for the random slopes.**
	    ...
	    sigma_ID_sd2 <- brms::posterior_samples(brms_personality_plasticity_dhglm_aggression, pars = "sd_id__sigma_age.Z")[,1]
...
rerun5 = FALSE 
	...
	if(rerun5){
		...
		brms_bivar_shglm_activity_aggression <- brm(formula_bivar_shglm_aggression + formula_bivar_shglm_activity, data = dat, 
								iter = 6000, warmup = 2000, chains = 3, cores = 3, 
								save_pars = save_pars()) 
		saveRDS(brms_bivar_shglm_activity_aggression, "./brms_models/brms_bivar_shglm_activity_aggression")
	} else {
		brms_bivar_shglm_activity_aggression <- readRDS(here::here("brms_models", "brms_bivar_shglm_activity_aggression"))
	}
...
####################################################################################
	...
	brms_bivar_dhglm_activity_aggression <- brm(formula_bivar_dhglm_aggression + formula_bivar_dhglm_activity, data = dat, 
							iter = 6000, warmup = 2000, chains = 3, cores = 3, 
							save_pars = save_pars()) 
	saveRDS(brms_bivar_dhglm_activity_aggression, "./brms_models/brms_bivar_dhglm_activity_aggression")

	} else {
		brms_bivar_dhglm_activity_aggression <- readRDS(here::here("brms_models", "brms_bivar_dhglm_activity_aggression"))
	}
